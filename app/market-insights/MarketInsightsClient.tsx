    ...
    return (
        <div className="market-page">
            ...
            <div className="stats-row">
                {[
                    { label: "Tổng việc làm", value: data.totalStats.totalJobs, color: "var(--primary)", bg: "rgba(59,130,246,0.1)", icon: "M21 13.255A23.931 23.931 0 0112 15c-3.183 0-6.22-.62-9-1.745M16 6V4a2 2 0 00-2-2h-4a2 2 0 00-2 2v2m4 6h.01M5 20h14a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" },
                    { label: "Công ty", value: data.totalStats.totalCompanies, color: "#3B82F6", bg: "rgba(59,130,246,0.1)", icon: "M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4" },
                    { label: "Ứng viên", value: data.totalStats.totalCandidates, color: "#F59E0B", bg: "rgba(245,158,11,0.1)", icon: "M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z" },
                    { label: "Lượt ứng tuyển", value: data.totalStats.totalApplications, color: "#10B981", bg: "rgba(16,185,129,0.1)", icon: "M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" },
                ].map(stat => (
            ...
            `}</style>
            <style jsx>{`
                .market-page { max-width: 1200px; margin: 0 auto; padding: 2.5rem 1.5rem 6rem; color: var(--text); font-family: var(--font-inter), sans-serif; }
                ...
                .h-svg { width: 36px; height: 36px; stroke: var(--primary); }
                .hero-header-wrap h1 { font-size: 2.25rem; font-weight: 900; color: var(--text); margin: 0; line-height: 1.1; letter-spacing: -0.02em; }
                .hero-subtitle { font-size: 1.05rem; color: var(--text-muted); line-height: 1.6; max-width: 480px; margin: 0; }
                ...
                .stat-card:hover { transform: translateY(-6px); box-shadow: 0 20px 25px -5px rgba(30, 64, 175, 0.1), 0 8px 10px -6px rgba(30, 64, 175, 0.1); border-color: var(--primary); }
                ...
                /* Cards */
                .card { background: var(--bg-card); border: 1px solid var(--border); border-radius: 24px; padding: 2rem; box-shadow: 0 4px 6px -1px rgba(0,0,0,0.02); transition: border-color 0.3s; }
                .card:hover { border-color: var(--primary); }
                .card-header { margin-bottom: 2rem; display: flex; justify-content: space-between; align-items: center; }
                .card-header h3 { font-size: 1.25rem; font-weight: 800; color: var(--text); margin: 0; letter-spacing: -0.01em; }
                ...
                .y-unit { position: absolute; top: -15px; left: 0; font-size: 0.65rem; font-weight: 800; color: var(--text-muted); }
                .trend-y-axis span:not(.y-unit) { font-size: 0.75rem; color: var(--text-muted); font-weight: 600; text-align: right; width: 35px; }
                .trend-bars { flex: 1; display: flex; align-items: flex-end; gap: 1.5rem; border-left: 1px dashed var(--border); border-bottom: 1px dashed var(--border); padding: 0 15px; position: relative; }
                .trend-bars::before { content: ''; position: absolute; top: 50%; left: 0; right: 0; height: 1px; border-top: 1px dashed var(--border); z-index: 0; }
                ...
                .bar-body { width: 100%; max-width: 40px; height: 0; background: linear-gradient(180deg, var(--primary) 0%, #1E40AF 100%); border-radius: 8px 8px 0 0; position: relative; transition: height 1s cubic-bezier(0.4, 0, 0.2, 1), transform 0.2s; box-shadow: 0 -4px 15px rgba(59, 130, 246, 0.15); }
                ...
                .bar-label { font-size: 0.8rem; color: var(--text-muted); font-weight: 700; margin-bottom: -28px; }
                ...
                .skill-info { display: flex; justify-content: space-between; font-size: 0.95rem; font-weight: 700; color: var(--text); }
                .skill-percent { color: var(--text-muted); font-variant-numeric: tabular-nums; }
                .progress-track { height: 10px; background: var(--border); border-radius: 12px; overflow: hidden; box-shadow: inset 0 1px 2px rgba(0,0,0,0.05); }
                ...
                .exp-card { background: var(--bg-card); padding: 1.5rem; border-radius: 20px; border: 1px solid var(--border); transition: all 0.2s; display: flex; flex-direction: column; justify-content: center; }
                .exp-card:hover { border-color: var(--primary); box-shadow: 0 10px 15px -3px rgba(0,0,0,0.05); transform: translateY(-2px); }
                .exp-label { font-size: 0.85rem; font-weight: 700; color: var(--text-muted); margin-bottom: 0.5rem; }
                .exp-count { font-size: 1.5rem; font-weight: 900; color: var(--text); display: flex; align-items: baseline; gap: 4px; }
                .exp-count small { font-size: 0.7rem; font-weight: 700; color: var(--text-muted); }
                .exp-bar { height: 6px; width: 100%; background: var(--border); border-radius: 10px; margin-top: 1rem; overflow: hidden; }
                ...
                .ind-rank { 
                    ...
                    background: rgba(59,130,246,0.15); width: 32px; height: 32px; border-radius: 10px; 
                    display: flex; align-items: center; justify-content: center; 
                    position: absolute; left: 0.5rem; top: 50%; transform: translateY(-50%);
                }
                .ind-content { display: flex; flex-direction: column; gap: 4px; }
                .ind-name { font-weight: 700; font-size: 1rem; color: var(--text); }
                .ind-count { font-size: 0.8rem; color: var(--text-muted); font-weight: 600; }
                ...
                .comp-item { display: flex; align-items: center; gap: 1.25rem; padding: 1rem; border-radius: 16px; border: 1px solid transparent; transition: all 0.2s; background: var(--bg-card); }
                .comp-item:hover { background: var(--bg-card); border-color: var(--primary); box-shadow: 0 4px 6px -1px rgba(59, 130, 246, 0.1); transform: translateX(4px); }
                .comp-logo { width: 56px; height: 56px; border-radius: 14px; background: var(--bg-card); border: 1px solid var(--border); display: flex; align-items: center; justify-content: center; overflow: hidden; flex-shrink: 0; }
                .comp-logo img { width: 100%; height: 100%; object-fit: cover; }
                .comp-logo span { font-weight: 900; color: var(--primary); font-size: 1.25rem; }
                .comp-name { font-weight: 800; font-size: 1rem; display: block; color: var(--text); margin-bottom: 4px; }
                ...
                .legend-item { display: flex; align-items: center; gap: 0.75rem; font-size: 0.85rem; font-weight: 700; color: var(--text-muted); transition: color 0.2s; cursor: pointer; }
                .legend-item:hover { color: var(--text); }
                .dot { width: 12px; height: 12px; border-radius: 4px; box-shadow: inset 0 0 0 1px rgba(0,0,0,0.1); }

                /* News Section */
                .news-section { margin-top: 5rem; border-top: 1px solid var(--border); padding-top: 4rem; }
                .news-header { display: flex; justify-content: space-between; align-items: center; margin-bottom: 2.5rem; }
                .news-header h2 { font-size: 2rem; font-weight: 900; color: var(--text); letter-spacing: -0.02em; margin: 0; }
                .view-all { font-weight: 700; color: var(--primary); font-size: 0.95rem; padding: 8px 16px; border-radius: 99px; background: rgba(59,130,246,0.1); transition: all 0.2s; }
                .view-all:hover { background: rgba(59,130,246,0.2); }
                .news-grid { display: grid; grid-template-columns: repeat(3, 1fr); gap: 2.5rem; }
                .news-card { background: var(--bg-card); border: 1px solid var(--border); border-radius: 24px; overflow: hidden; transition: all 0.3s; display: flex; flex-direction: column; }
                .news-card:hover { transform: translateY(-8px); box-shadow: 0 20px 25px -5px rgba(0,0,0,0.05); border-color: var(--primary); }
                .news-img { height: 200px; background: var(--border); overflow: hidden; }
                ...
                .news-body h3 { font-size: 1.15rem; font-weight: 800; margin-bottom: 1rem; line-height: 1.5; color: var(--text); display: -webkit-box; -webkit-line-clamp: 2; -webkit-box-orient: vertical; overflow: hidden; }
                .news-body p { font-size: 0.9rem; color: var(--text-muted); line-height: 1.6; margin-bottom: 1.5rem; display: -webkit-box; -webkit-line-clamp: 2; -webkit-box-orient: vertical; overflow: hidden; flex: 1; }
                .news-date { font-size: 0.8rem; color: var(--text-muted); font-weight: 700; }
